...
const files = ['OPERACAO.md', 'scripts/migrate.cjs', 'scripts/integrate.cjs', 'ui/app.js', 'ui/admin.css', 'ui/index.html', 'config/admin-panel.example.json', 'migrations/001_admin_panel.sql', ...fs.readdirSync(path.join(root, 'gamemode')).filter(file => file.endsWith('.cjs')).map(file => `gamemode/${file}`)];
...
fs.writeFileSync(path.join(target, 'manifest.json'), JSON.stringify({ version: '0.1.0', files: manifest }, null, 2));
