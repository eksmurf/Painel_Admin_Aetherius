...
const files = ['README.md', 'OPERACAO.md', 'INTEGRACAO_INGAME.md', 'PROMPT_INSTALACAO.md', 'scripts/migrate.cjs', 'scripts/integrate.cjs', 'client/AdminToolsService.ts', 'ui/app.js', 'ui/admin.css', 'ui/index.html', 'config/admin-panel.example.json', 'migrations/001_admin_panel.sql', ...fs.readdirSync(path.join(root, 'gamemode')).filter(file => file.endsWith('.cjs')).map(file => `gamemode/${file}`), ...fs.readdirSync(path.join(root, 'third-party/alduinak')).filter(file => /\.(md|txt)$/.test(file)).map(file => `third-party/alduinak/${file}`)];
