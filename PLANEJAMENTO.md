...
| Jogadores | Busca por nome/identificador, lista paginada, estado da conexão, seleção inequívoca do alvo |
| Detalhes do jogador | Nome, identificadores necessários ao atendimento, localização permitida, ações disponíveis |
...
O fluxo existente recebe `cef::ui:event` no cliente, envia `{type, data}` em `CustomPacket` e resolve `actorId` por `mp.getUserActor(userId)` no servidor. A extensão proposta registra exclusivamente o namespace `admin` em `core/ui-event-router.js`.

Proposta de eventos, ainda não implementados: `admin:open`, `admin:players:list`, `admin:action`, `admin:audit:list` e `admin:close`. O handler deve rejeitar eventos fora de seu namespace: o roteador atual também invoca handlers de outros prefixos como fallback.
...
Manter `teleport`, `kick`, `add_item`, `set_gold` e `view_audit`; adicionar capacidades explícitas para abertura/leitura conforme o catálogo. A atribuição proposta deve ficar versionada no servidor. A tabela `staff_permissions` existe no schema, mas o serviço examinado deriva permissões do mapa em código; não presumir que editar essa tabela altere os acessos.
...
Registrar tentativa autorizada antes da mutação e resultado após execução: `requestId`, origem (chat/painel), conta operadora, cargo observado, alvo, sessão, parâmetros normalizados, motivo, horário do servidor, estado anterior/posterior quando cabível e erro sanitizado.
...
Pontos esperados de integração em `Aetherius-RP-Local`: `aetherius/ui/index.html`, `aetherius/gamemode/phase0-basic.js`, `commands.js`, `admin-service.js`, `core/ui-event-router.js`, TypeScript da ponte/foco e `scripts/Prepare-Runtime.ps1`. Existem alterações locais no monorepo; preservar e identificar o diff da implementação separado delas.
...
|---|---|---|---|
| M0 — Contratos reais | Inventariar artefatos usados, provar CEF → servidor → CEF; confirmar identidade, IDs e APIs; definir integração de fontes | Dois clientes recebem respostas privadas e correlacionadas; lista de comandos homologáveis | 2–3 dias |
| M1 — Serviço autorizado | Catálogo, schemas, permissões, revogação, auditoria, deduplicação; corrigir kick e retornos; adaptar chat | Pacote forjado, replay e usuário comum não alteram estado; sem bypass pelo chat | 3–5 dias |
...
Total estimado: 15–23 dias úteis para a primeira versão com itens e economia. Um MVP de consulta/teleporte/kick pode ser entregue após M3 e uma rodada proporcional dos testes de M5. O serviço de identidade de produção é um bloqueio de publicação, mesmo que a interface esteja concluída.

Trazer/retornar, banimentos, animações, cura/reviver, ferramentas de mundo e gestão de cargos formam etapas posteriores. Cada uma deve passar pela matriz de viabilidade antes de receber um botão operacional.
